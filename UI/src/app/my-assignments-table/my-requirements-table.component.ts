...
export class MyRequirementsTableComponent implements AfterViewInit {
  ...
  expandedElement: Requirement | null;

  @ViewChild(MatPaginator) paginator: MatPaginator;

  constructor(private requirementService: RequirementService) {}

  requirementMapper(data: Requirement[][][]): Requirement[] {
    let exams = data[0][0];
    let assignments = data[0][1];
    return exams.concat(assignments);
  }

  ngAfterViewInit() {
    this.requirementService.getRequirements().subscribe((data) => {
      this.dataSource.data = this.requirementMapper(data) || [];
